...
def diabetespred():
    int_features=[float(x) for x in request.form.values()]
    final=[np.array(int_features)]
    prediction_prob = diabetes_model.predict_proba(final)[:, 1]  # Get the probability for the positive class

    # Set a threshold (e.g., 0.5), and check if the probability exceeds it
    threshold = 0.5
...
def cancerpred():
    int_features=[float(x) for x in request.form.values()]
    final=[np.array(int_features)]
    prediction_prob = cancer_model.predict_proba(final)[:, 1]  # Get the probability for the positive class
...
def vectorpred():
    ...
    # Define a list of class labels in the same order as your model's output
